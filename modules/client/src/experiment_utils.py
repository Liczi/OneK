...
from utils import get_actual_state, extract_players, get_stage_winner, get_game_winner
...
def test_agents(agent_names, agents, games, stateful_game=None):
    ...
    for _ in tqdm(range(0, games)):
        ...
        if stateful_game:
            res = run_stateful_game(players, state, stateful_game)
        else:
            res = run_game(players, state)
